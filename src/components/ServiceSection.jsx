...
// const ServiceSection = ({ selectedServices, setSelectedServices, uuid,
//   department,
//   consultingDoctor,
//   paymentMode,
//   setBillingTotals
...
//   const canAdd = uuid && department && consultingDoctor;
...
//                   <th className="p-2 border">Net Charge</th>
...
//                     <td className="p-2 border text-left">{service.name}</td>
...
const ServiceSection = ({
  ...
  const handleAddService = (service) => {
    if (!canAdd) {
      healthAlert({
        title: "UHID Required",
        text: "Please select a UHID from the list",
        icon: "warning",
      });
      return;
    }
  ...
  // ⚡ Auto calculate billing totals when selected services change
  useEffect(() => {
    ...
    setBillingTotals((prev) => ({
      ...prev,
      totalBilling: total,
      cashAmount: paymentMode === "Cash" ? total : 0,
      cardAmount: paymentMode !== "Cash" ? total : 0,
    }));
  }, [selectedServices, paymentMode]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="col-span-2 relative">
          ...
          <input
            ...
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className={`border rounded-lg px-3 py-2 w-full ${!canAdd ? "bg-gray-100 cursor-not-allowed" : ""
              }`}
            disabled={!canAdd}
          />
