...
const BillingFormCopy = ({ refetchList }) => {
  ...
  const { data: patientData, refetch: refetchPatientData } =
    useGetOpdBillByIdQuery(selectedBill ? String(selectedBill) : skipToken);
  ...
  const { data: billData, refetch: refetchBillData } =
    useGetMedicineBillByIdQuery(id, {
      skip: !id,
    });

  const { data: medicineResponse, refetch: refetchMedicineList } =
    useGetMediceneListQuery(
  ...
  const { data: suggestions = [], refetch: refetchBillSearch } =
    useSearchOpdBillNoQuery(debouncedUhid, {
      skip: debouncedUhid.length < 1 || id,
    });
  const { data: paymodes, refetch: refetchPaymode } =
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const paymodeId = values.payMode;
      const payload = {
        ...values,
        ...
        items: values.items.map((i) => ({
          ...
          discAmt: Number(paymodeId == 5 ? values.paidAmount : i.discAmt || 0),
