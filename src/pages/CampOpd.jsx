...
const CampOpdForm = () => {
  ...
  const [createBill, { isLoading: isCreating, error: createError }] =
    useCreatecampBillMutation();
  const [updateBill] = useUpdatecampBillMutation();
  ...
  const buildPayload = (values) => {
    ...
    const chiefComplaintStr =
      Array.isArray(values.ChiefComplaint) && values.ChiefComplaint.length > 0
        ...
        : "";
    //      const referDoctorName =
    // doctors?.find((d) => d.id == values.ReferBy)?.name ||
    // doctors?.find((d) => d.id == values.ReferBy)?.doctor_name ||
    // "";
    const header = {
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-blue-100 border border-gray-100 overflow-hidden">
          ...
          <div className="p-10">
            ...
            >
                ...
                <section>
                  <ServiceSection
                    ...
                    setBillingTotals={(total) => {
                      ...
                      //                         const currentPaid = Number(formik.values.PaidAmount || 0);
                      // const oldTotal = Number(formik.values.TotalAmount || 0);
                      ...
                      //  if (!editData && currentPaid === 0) {
                      //   updatedPaid = amount;
                      // }

                      //   formik.setFieldValue("PaidAmount", updatedPaid);

                      //   if (
                      //     formik.values.PayMode === "1" ||
                      //     formik.values.PayMode === ""
                      //   ) {
                      //     formik.setFieldValue("CashAmount", updatedPaid);
                      //     formik.setFieldValue("CardAmount", 0);
                      //   } else {
                      //     formik.setFieldValue("CardAmount", updatedPaid);
                      //     formik.setFieldValue("CashAmount", 0);
                      //   }
                      // }
                    }}
              ...
              {activeStep === 4 && (
                <>
                  <div className="bg-blue-50 p-6 rounded-xl border border-blue-200 space-y-5">
                    ...
                    <div>
                      ...
                      {formik.values.ChiefComplaint?.length > 0 ? (
                        <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                          {formik.values.ChiefComplaint.map((c, idx) => (
