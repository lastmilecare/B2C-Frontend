...
const CampOpdBillingList = () => {
  ...
  const handleDelete = async (row) => {
    ...
    try {
      ...
      healthAlert({
        title: "Deleted!",
        text: "Camp OPD Bill Deleted Successfully.",
        icon: "success",
      });
    } catch (error) {
      healthAlert({
  ...
  const handleEdit = (row) => {
    ...
    navigate(`/camp-opd-form/${row.bill_no}`, {
      state: {
        editData: row,
      },
    });
  ...
  const { data, isLoading, isError, error } = useGetcampOpdBillingQuery(
    {
      page,
      limit,
      ...filters,
...
export default CampOpdBillingList;
