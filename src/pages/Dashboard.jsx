...
const DashboardCopy = () => {
    ...
    const { data: prescriptionData } = useGetPrescriptionsListQuery({ page: 1, limit: 100 })
    ...
    const todayOpd = opd.filter(o =>
        new Date(o.AddedDate).toDateString() === today
    ).length

    const todayPrescription = prescriptions.filter(p =>
        new Date(p.addedDate).toDateString() === today
    ).length
            ...
            <div className="bg-gradient-to-r from-emerald-600 via-emerald-350 to-emerald-600 text-white rounded-2xl p-8 flex justify-between items-center shadow-lg">
                ...
                <div>
                    ...
                    <p className="opacity-90">
                        LMC Healthcare Management System
                    </p>
