...
const PrescriptionListCamp = () => {
  ...
  const { data: billSuggestions = [] } = useSearchcampOpdBillNoQuery(
    debouncedBillSearch,
    {
      skip: debouncedBillSearch.length < 1,
    },
  );
  const [toggleStatus] = useToggleCampPrescriptionStatusMutation();
  const [filters, setFilters] = useState({});
  const [printRow, setPrintRow] = useState(null);
  const printRef = useRef();
  const { data, isLoading, isError, error } = useGetCampPrescriptionsListQuery(
    {
      page,
      limit,
      ...filters,
...
//     },
{
  ...
  const handleEdit = (row) => {
    ...
    navigate(`/prescription-form-camp/${row.ID}`, {
      state: { row },
    });
  };
...
export default PrescriptionListCamp;
